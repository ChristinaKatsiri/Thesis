...
reviews_df = pd.read_csv("C:/Users/katsi/Desktop/paradotea_local/Hotel_Reviews_1.csv", sep = ',')
reviews_df = reviews_df.dropna(subset=['review_tag_vader'])

# Now X can be used in your machine learning model
label_encoder = LabelEncoder()
y_encoded = label_encoder.fit_transform(reviews_df['review_tag_vader'])
...
for _, row in reviews_df.iterrows():
    ...
    review_class = row['review_tag_vader']
...
print(predicted_probs_wu_palmer)
...
# Compute ROC curve and ROC area for each class
fpr_path, tpr_path, _ = roc_curve(y_binarized_path.ravel(), predicted_probs_path.ravel())
...
# Compute ROC curve and ROC area for each class
fpr_res, tpr_res, _ = roc_curve(y_binarized_res.ravel(), predicted_probs_res.ravel())
...
# Compute ROC curve and ROC area for each class
fpr_lch, tpr_lch, _ = roc_curve(y_binarized_lch.ravel(), predicted_probs_lch.ravel())
...
precision_recall_lch_data = {'Precision_lch': precision_lch, 'Recall_lch': recall_lch} #, 'Thresholds_lch': thresholds_lch}
...
# You can convert these data structures to dataframes for easier manipulation or export
precision_recall_wu_palmer_data = {'Precision_wu_palmer': precision_wu_palmer, 'Recall_wu_palmer': recall_wu_palmer} #, 'Thresholds_wu_palmer': thresholds_wu_palmer}
...
# You can convert these data structures to dataframes for easier manipulation or export
precision_recall_path_data = {'Precision_path': precision_path, 'Recall_path': recall_path} #, 'Thresholds_path': thresholds_path}
...
precision_recall_res_data = {'Precision_res': precision_res, 'Recall_res': recall_res}#, 'Thresholds_res': thresholds_res}
